...
%Solve for Case 1 - With no Control measures
...
%calculatig population of S,I and R over time for entire State
for t=1:60
S_loop_OH(t)=0;
L_loop_OH(t)=0;
I_loop_OH(t)=0;
...
%calculatig population of S,I and R over time for entire State for the true data
for t=1:45
for index=1:88
...
%Solve for Case 3 - With Control measures
...
if k==1
 ...
 colormap(jet(256));
     title(sprintf('Infectious Population @ Day  %d- Real Data', (k)*15))
end
...
env_I{3}(j,i)=I_loop{j,i}(30);
env_I{4}(j,i)=I_loop{j,i}(45);
env_I{5}(j,i)=I_loop{j,i}(60);
    end
end
...
if k==1
 ...
 colormap(jet(256));
     title(sprintf('Infectious Population @ Day  %d- Model Results - Case 1', (k-1)*15))
end
...
env_I{3}(j,i)=I_loop_1{j,i}(30);
env_I{4}(j,i)=I_loop_1{j,i}(45);
env_I{5}(j,i)=I_loop_1{j,i}(60);
    end
end
...
if k==1
 ...
 colormap(jet(256));
     title(sprintf('Infectious Population @ Day  %d- Model Results - Case 2', (k-1)*15))
end
...
env_I{3}(j,i)=I_loop_2{j,i}(30);
env_I{4}(j,i)=I_loop_2{j,i}(45);
env_I{5}(j,i)=I_loop_2{j,i}(60);
    end
end
...
if k==1
 ...
 colormap(jet(256));
     title(sprintf('Infectious Population @ Day  %d- Model Results - Case 3', (k-1)*15))
end

for i=1:23
   saveas(figure(i),sprintf('Figure %d.eps',i),'epsc'); 
end