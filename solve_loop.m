...
counter=0
%find out all the cells which are on the boundry and make an array of their closest in-map neighbours
for i=1:param.grid
    for j=1:param.grid
...
u_2=1;
for T=1:iter
%Implement the value for u_2 for each cell, the control parameter depending on the case being simulated
       ...
       for j=2:param.grid-1
          if (env(j,i).in_map==1)
              if T>15
               ...
               %Latency calculation for each cell
                  latency=0;
                  if T<=param.tau %If time is less than tau, calculate latency value from the 1st day vaue of S and I 
                      for k =1:param.grid   
                         for l=1:param.grid
                  ...
                  else %If time is greater than tau, calculate latency value from T-tau day for S and I
                     for k =1:param.grid   
                         for l=1:param.grid
                             if (env(k,l).in_map==1&&env(k,l).boundry==0)
                             fa=(1/(sqrt(4*pi*param.alpha)))*exp(-((i-l)^2+(j-k)^2)/(4*param.alpha));
        ...
        % update the values for S,I,R and L for each cell at the next time instant
    ...
    %Apply the reflective boundry condition for all the cells on the boundry
